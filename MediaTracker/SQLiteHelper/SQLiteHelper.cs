...
namespace System.Data.SQLite
{
    ...
    public class SqLiteHelper
    {
        ...
        public TDataType ExecuteScalar<TDataType>(string sql, Dictionary<string, object> dicParameters = null)
        {
            ...
            return ExecuteScalar<TDataType>(sql, lst);
        }

        public TDataType ExecuteScalar<TDataType>(string sql, IEnumerable<SQLiteParameter> parameters = null)
        {
            cmd.CommandText = sql;
            if (parameters != null)
            {
            ...
            return (TDataType)Convert.ChangeType(cmd.ExecuteScalar(), typeof(TDataType));
        }

        public TDataType ExecuteScalar<TDataType>(string sql)
        {
            cmd.CommandText = sql;
            return (TDataType)Convert.ChangeType(cmd.ExecuteScalar(), typeof(TDataType));
        }
        ...
        public void Update(string tableName, Dictionary<string, object> dicData, Dictionary<string, object> dicCond)
        {
            ...
            Dictionary<string, object> dicTypeSource = new Dictionary<string, object>();

            foreach (KeyValuePair<string, object> kv1 in dicData)
            {
                dicTypeSource[kv1.Key] = null;
            }

            foreach (KeyValuePair<string, object> kv2 in dicCond)
            {
                if (!dicTypeSource.ContainsKey(kv2.Key))
                    dicTypeSource[kv2.Key] = null;
            }
